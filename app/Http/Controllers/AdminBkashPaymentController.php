...
class AdminBkashPaymentController extends Controller
{
    private $base_url;
    private $app_key;
    private $app_secret;
    ...
    public function createPayment($testimonial_id)
    {
        ...
        $response = Http::withHeaders([
            ...
            'mode' => '0011',
            'payerReference' => 'ADMIN-' . $admin->id,
            'callbackURL' => route('admin.bkash.callback', $testimonial->id),
            'amount' => $testimonial->payment_amount,
            'currency' => 'BDT',
            'intent' => 'sale',
            'merchantInvoiceNumber' => 'ADMIN-TEST-' . $testimonial->id . '-' . time(),
        ]);
        ...
        if ($response->successful()) {
            ...
            session([
                'admin_bkash_payment_id' => $data['paymentID'],
                'admin_testimonial_id' => $testimonial->id,
            ]);

            // Log payment transaction with admin reference
